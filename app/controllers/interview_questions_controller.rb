class InterviewQuestionsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @interview_question.update(interview_question_params)
        format.html { redirect_to interview_path(@interview_question.interview_id), notice: 'Interview question was successfully updated.' }
  ...
  private
    ...
    def interview_question_params
        params.require(:interview_question).permit(:interview_id, :question_id, :responce)
    end
end
